...
const BANDS: Band[] = [
  { label: 'THE NAME, AS THE PIT SETS IT', text: 'THE SUMMONING PIT', blend: 2.6, hand: 1 },
  ...
  { label: 'HAND 0 — MACHINED, FOR REFERENCE', text: 'HAMMERED', blend: 0.4, hand: 0 },
  { label: 'HAND 2.4 — BEATEN IN', text: 'HAMMERED', blend: 0.4, hand: 2.4 },
  { label: 'BLUR 0 / 2 / 5 — THE FIELD DOING THE WORK', text: 'PIT', blend: 0, hand: 1 },
...
BANDS.forEach((b, i) => {
  ...
  const ppm = Math.min(W * 0.82 / Math.max(0.5, t.width), BAND * 0.52);
  const cam: Camera = {
    yaw: 0, pitch: 0, ppm, cx: 0, cz: 0, cy: 0.5,
